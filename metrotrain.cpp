...
public:
    ...
    {
        cout << "\nThe train will operate now for " << stationsCount << " stations \n";
    ...
    void printStatistics()
    {
        /***********************************************************************
         * ToDo:
         * - What is the total count of passengers that entered the train
         * - How many had a reduced ticket
         * ********************************************************************/
...
{
    srand(time(0));         //In order to use rand later in our functions
    using std::ifstream;
    using std::endl;
    using std::cerr;
    /**************************************************************************
     * ToDo: 
     * - Enter interactive mode when command line argument '-i' is given and 
     * exit the program if no argument, providing usage insturctions.
     * ************************************************************************/
    
    int stationsCount(0), wagonsCount(10), wagonMaxCapacity(30) ;
	...
	{
	    // set up a stringstream variable named convert1, initialized with the input from argv[1]
    	std::stringstream convert1(argv[1]); 
 
    	if (!(convert1 >> stationsCount) || stationsCount < 0) // do the conversion
    	{
    	    cerr << "The input given for stations count seems invalid. Please try again, "
    	...
    	if(argc >= 3)
    	{
    	    // set up a stringstream variable named convert2, initialized with the input from argv[1]
        	std::stringstream convert2(argv[2]); 
     
        	if (!(convert2 >> wagonsCount) || wagonsCount < 0) // do the conversion
        	{
        	    cerr << "The input given for wagons count seems invalid. Please try again, "
        	            << "entering a positive integer\n";
                return 1;
        	}
    	}
    	if(argc >= 4)
    	{
    	    // set up a stringstream variable named convert3, initialized with the input from argv[1]
        	std::stringstream convert3(argv[3]); 
     
        	if (!(convert3 >> wagonMaxCapacity) || wagonMaxCapacity < 0) // do the conversion
        	{
        	    cerr << "The input given for wagons count seems invalid. Please try again, "
        	            << "entering a positive integer\n";
                return 1;
    ...
    //Operate the metroTrain for stationsCount stations
